...
# 情感向量 8 维顺序，与 IndexTTS2.5 normalize_emo_vec 一致
...
def _concatenate_wav_segments(segments, output_path: str):
    ...
    22.05 kHz，遇到格式不一致时立即失败，避免生成播放器无法解析的成品。
    """
...
def _apply_loudness(path: str) -> None:
    """只做响度归一化，语速由 IndexTTS-2.5 原生 duration_factor 控制。"""
    ...
    result = subprocess.run(
        [ffmpeg, "-y", "-i", path, "-filter:a", "loudnorm=I=-16:TP=-1.5:LRA=11",
         "-ar", str(OUTPUT_SAMPLE_RATE), tmp],
...
def synthesize_podcast(
    ...
    try:
        ...
        def _run_one(job: dict) -> dict:
            ...
            _apply_loudness(job["segment_path"])
            t_end = time.time()
            logger.info(
