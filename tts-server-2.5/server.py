"""IndexTTS-2.5 TTS 服务端。

保持与现有 tts-server 相同的 HTTP API；2.5 特有逻辑隔离在本目录。

将 IndexTTS2 包装为 HTTP API，供 WebUI 后端调用。
部署在与 index-tts 源码同一台 GPU 服务器上。

启动方式：
  python server.py --indextts-home /root/index-tts-2.5 \
...
parser = argparse.ArgumentParser(description="IndexTTS-2.5 TTS Server")
...
parser.add_argument("--model-dir", default=os.environ.get("MODEL_DIR", "/mnt/storage/index-tts-data/checkpoints_25"),
                    help="IndexTTS-2.5 模型权重目录")
parser.add_argument("--voices-dir", default=os.environ.get("VOICES_DIR", "/mnt/storage/index-tts-data/voices"),
                    help="参考音频存放目录")
parser.add_argument("--output-dir", default=os.environ.get("OUTPUT_DIR", "/mnt/storage/index-tts-data/outputs_25"),
                    help="2.5 生成音频输出目录")
parser.add_argument("--device", default=os.environ.get("DEVICE", "cuda:0"), help="推理设备")
parser.add_argument("--bf16", action="store_true", default=os.environ.get("USE_BF16", "1") == "1",
                    help="启用 BF16（IndexTTS-2.5 推荐）")
...
parser.add_argument("--port", type=int, default=int(os.environ.get("PORT", "8001")), help="监听端口")
...
app = FastAPI(title="IndexTTS-2.5 TTS Server", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def synthesize(req: SynthesizeRequestModel):
    ...
    infer_kwargs.update(gen.to_infer_kwargs(speed=req.params.speed))

    with _model_lock:
        try:
            tts.infer(**infer_kwargs)
            from podcast_engine import _apply_loudness
            _apply_loudness(output_path)
        except Exception as e:
